Оригинальный README начинается ниже. Здесь перечисленны изменения, внесенные в оригинальный репозиторий.\
\
\
\
**Изменения в .cfg файлах (файлах с конфигурацией модели и параметрами обучения/тестирования)**
* Добавлен параметр **min_box_area** в секции **[net]**. При обучении все боксы с площадью ниже значения, указанного в данном параметре, будут отброшенны. Площадь боксов считается на изображении, подготовленном для подачи на вход сети.
* Добавлен параметр **init_layers** в секции **[net]**. При обучении с параметром -clear, из предобученной модели будут загруженны первые init_layers слоев. Если значение init_layers равно 0, то будет загруженна вся предобученная сеть.
* Добавлен параметр **freeze_layers** в секции **[net]**. Указывает, сколько нужно замораживать слоев при заморозке сети.
* Добавлен параметр **when_defrost** в секции **[net]**. На протяжении первых when_defrost батчей обучения, первые freeze_layers слоев сети будут заморожены. То есть сеть размораживается после when_defrost батчей.
...
\
**predict.py**\
Прогоняет изображения через модель и сохраняет результаты в файл.
```bash
Параметры:
-cfg, --config-file                                   - конфигурационный файл модели
-model, --model-file                                  - файл с весами модели
-img-fld, --images-folder                             - папка с изображениями или папка, относительно которой задаются пути к изображениям в файле (см. параметр -img)
-out, --out-file                                      - выходной файл с результатами детекций
-to, --predict-to [choices coco, cvat] [default coco] - в каком формате сохранять результаты детекций
-dets-only, --detections-only [optional]              - если результаты детекций сохраняются в формате coco (см. параметр -to), то сохранены будут только детекции (без изображений и категорий)
-img, --images-file [optional]                        - если ничего не указано, то изображения для детекции будут браться из папки -img-fld. Если указать *.json файл со списком изображений
                                                        в формате coco (например файл с аннотациями), то пути к изображениям будут браться из этого файла, а в параметре -img-fld нужно указать
							путь к папке, относительно которой задаются пути к изображениям в *.json файле. При этом id изображений в выходном файле с результатами
							будут совпадать с id изображений в указанном в данном параметре *.json файле
-cls, --classes-file [optional]                       - путь к файлу с названиями классов. Если ничего не указано - скрипт не работает. Нужно указать путь либо к *.json файлу со списком
                                                        категорий в формате coco (например файлу с аннотациями), либо путь к *.names файлу с названиями классов каждый с новой строки
							(данный файл нужен при обучении сетей в darknet)
-thr, --threshold [default 0.001]                     - порог по скору для детекций. Не рекомендуется ставить 0, так как будет получаться очень большое количество боксов и это сильно замедлит
                                                        процесс обработки результатов
-max-dets, --max-dets [default 1000]                  - максимальное количество боксов на одном изображении.
-is, --input-shape [default None None]                - входной размер сети (ширина и высота). Если используется значение по умолчанию (None None), то будет использованно входное расширение
                                                        из конфигурационного файла (см. парамерт -cfg)
-gpu, --gpu [default 0]                               - номер GPU (как указано в nvidia-smi), на которую загружать модель и проводить вычисления
```
\
\
**report.py**\
Валидирует промежуточные модели, полученные при обучении, сохраняет результаты вылидации и стоит графики.
```bash
Параметры:
-cfg, --config-file                          - конфигурационный файл моделей
-models-fld, --models-folder                 - папка, в которой находятся файлы с весами промежуточных моделей для валидации. Названия файлов с весами должны соответствовать шаблону
                                               epoch_N.weights, где N - номер эпохи
-rep-fld, --report-folder                    - папка, в которой будут сохранены результаты валидации
-img-fld, --images-folder                    - папка, относительно которой задаются пути к изображениям в файле с аннотациями (см. параметр -ann)
-ann, --annotations-file                     - путь к файлу с аннотациями
-area, --area [default 0**2 1e5**2]          - перед вычислением метрик из детекций и аннотаций будут удалены боксы, площадь которых находится вне диапазона, указанного в данном параметре (см. также параметр -shape).
                                               В качестве верхнего предела можно указать -1, что будет эквивалентно 1e5**2
-shape, --shape [default None None]          - если используется значение по умолчанию (None None), то площадь боксов берется на исходном изображении. Если передать два числа, то перед отбасыванием
                                               неподходящих по размеру боксов, изображение будет отмаштабированно с сохранением соотношения сторон так, чтобы влазить в рамки с шириной и высотой,
				               взятыми из параметра -shape. Площади боксов будут пересчитаны соответствующим образом.
-add, --add [optional]                       - если указано, то перед валидацией из папки -rep-fld будут прочитаны уже готовые результаты и провалидированные промежуточные модели повторно валидироваться не будут.
-dont-repredict, --dont-repredict [optional] - если указано, то при обнаружении в папке -rep-fld файла с детекциями для какой-нибудь промежуточной модели и отсутствии вычисленных для нее метрик
                                               (такое может быть, если работа скрипта была прервана), то повторная валидация для этой модели проводиться не будет.
-gpu, --gpu [default 0]                      - номер GPU (как указано в nvidia-smi), на которую загружать модели и проводить вычисления
```
\
\
\
Далее идет README из оригинального репозитория
